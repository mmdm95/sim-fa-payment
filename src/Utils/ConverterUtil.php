...
class ConverterUtil
{
    ...
    const CONVERT_ALL = ConverterUtil::CONVERT_NUMBERS | ConverterUtil::CONVERT_CHARACTERS;
     ...
     * @return array|mixed
     */
    public static function toPersian($str, $convert_type = ConverterUtil::CONVERT_ALL)
    {
        if (ConverterUtil::CONVERT_NUMBERS & $convert_type || ConverterUtil::CONVERT_ALL & $convert_type) {
            if (\is_array($str)) {
                $newArr = [];
                foreach ($str as $k => $v) {
        ...
        } elseif (ConverterUtil::CONVERT_CHARACTERS & $convert_type || ConverterUtil::CONVERT_ALL & $convert_type) {
            if (\is_array($str)) {
                $newArr = [];
                foreach ($str as $k => $v) {
                    $newArr[$k] = self::toPersian($v, $convert_type);
     ...
     * @return array|mixed
     */
    public static function toArabic($str, $convert_type = ConverterUtil::CONVERT_ALL)
    {
        if (ConverterUtil::CONVERT_NUMBERS & $convert_type || ConverterUtil::CONVERT_ALL & $convert_type) {
            if (\is_array($str)) {
                $newArr = [];
